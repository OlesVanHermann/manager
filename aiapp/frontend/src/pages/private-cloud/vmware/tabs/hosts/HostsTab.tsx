...
export default function HostsTab({ serviceId }: { serviceId: string }) {
  const { t } = useTranslation("private-cloud/vmware/hosts");
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      setError(null);
      setHosts(await hostsService.getHosts(serviceId));
    } catch (e) {
  ...
  if (!hosts.length) return <div className="hosts-empty"><h2>{t("empty.title")}</h2><p>{t("empty.description")}</p></div>;
        ...
        <tbody>
          {hosts.map((h) => (
            <tr key={h.hostId}>
              ...
              <td>{h.profile}</td>
              <td>{h.cpu}</td>
              <td>{formatRam(h.ram)}</td>
              <td><span className={`status-badge ${getHostStateBadgeClass(h.state)}`}>{h.state}</span></td>
