...
import { getCluster } from "./tabs/GeneralTab.service";
...
export default function KubernetesPage() {
  ...
  const loadCluster = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await getCluster(projectId, clusterId);
      setCluster(data);
    } catch (err) {
    ...
    return (
      <div className="page-content">
        <div className="error-state">
          <p>{error}</p>
          <button className="btn btn-primary" onClick={loadCluster}>{t("error.retry")}</button>
        </div>
      </div>
    );
  }
