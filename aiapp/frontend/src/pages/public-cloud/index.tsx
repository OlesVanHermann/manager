...
export default function PublicCloudDashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const projectIds = await cloudService.listProjects().catch(() => []);
        let instances = 0, volumes = 0, containers = 0;
        for (const projectId of projectIds.slice(0, 5)) {
          const [inst, vol, cont] = await Promise.all([
            cloudService.listInstances(projectId).catch(() => []),
            cloudService.listVolumes(projectId).catch(() => []),
            cloudService.listContainers(projectId).catch(() => []),
          ]);
