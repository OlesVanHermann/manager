...
interface Task { id: number; function: string; status: string; startDate: string; doneDate?: string; }
...
export default function TasksTab({ serviceId }: TasksTabProps) {
  ...
  const loadTasks = async () => {
    try { setLoading(true); setError(null); const data = await cloudConnectService.getTasks(serviceId); setTasks(data); }
    ...
    <div className="tasks-tab">
      ...
      {tasks.length === 0 ? <div className="empty-state"><h2>{t("tasks.empty.title")}</h2></div> : (
        <table className="data-table">
          ...
          <tbody>
            {tasks.map((task) => (<tr key={task.id}><td>{task.function}</td><td>{getStatusBadge(task.status)}</td><td>{new Date(task.startDate).toLocaleString("fr-FR")}</td><td>{task.doneDate ? new Date(task.doneDate).toLocaleString("fr-FR") : "-"}</td></tr>))}
          </tbody>
        </table>
      )}
    </div>
