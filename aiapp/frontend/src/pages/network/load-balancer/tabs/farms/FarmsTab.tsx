...
export function FarmsTab({ serviceName }: Props) {
  ...
  useEffect(() => {
    const load = async () => {
      try { setLoading(true); const data = await farmsService.getAllFarms(serviceName); setFarms(data); }
  ...
  if (loading) return <div className="farms-loading"><div className="skeleton-block" /></div>;

  return (
    <div className="farms-tab">
      <div className="farms-header"><h3>{t("farms.title")}</h3><span className="farms-count">{farms.length}</span></div>
      {farms.length === 0 ? (<div className="farms-empty"><p>{t("farms.empty")}</p></div>) : (
        <div className="farms-list">
          {farms.map(farm => (
            <div key={farm.farmId} className={`farms-card ${expandedFarm === farm.farmId ? "expanded" : ""}`}>
              <div className="farms-card-header" onClick={() => loadServers(farm.farmId)}>
                ...
                <svg className="farms-chevron" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg>
              </div>
              {expandedFarm === farm.farmId && farm.servers && (
                <div className="farms-servers">
                  {farm.servers.length === 0 ? (<p className="farms-no-servers">{t("farms.noServers")}</p>) : (
                      ...
                      <tbody>
                        {farm.servers.map(s => (
                          <tr key={s.serverId}>
                            ...
                            <td>{s.port || "-"}</td>
                            <td>{s.weight || "-"}</td>
                            <td><span className={`farms-server-status ${s.status === "active" ? "success" : "inactive"}`}>{s.status}</span></td>
                            <td>{s.ssl ? "✓" : "✗"}</td>
                          </tr>
                        ))}
                      </tbody>
