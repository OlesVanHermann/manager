...
export default function VirtuozzoLicensePage() {
  ...
  const tabs = [{ id: "general", label: t("virtuozzo.tabs.general") }, { id: "tasks", label: t("virtuozzo.tabs.tasks") }];
  const { activeTab, TabButtons } = useTabs(tabs, "general");

  useEffect(() => { if (!licenseId) { setLoading(false); return; } loadLicense(); }, [licenseId]);
  const loadLicense = async () => { try { setLoading(true); setError(null); const data = await licenseService.getVirtuozzoLicense(licenseId); setLicense(data); } catch (err) { setError(err instanceof Error ? err.message : "Erreur"); } finally { setLoading(false); } };
  ...
  if (!licenseId) return <div className="page-content"><div className="empty-state"><h2>{t("virtuozzo.noLicense.title")}</h2></div></div>;
  ...
  return (
    <div className="page-content license-page">
      <header className="page-header"><h1>📦 {t("types.virtuozzo")} - {license?.ip}</h1>{license && <div className="service-meta"><span className="meta-item">Version: {license.version}</span><span className="meta-item">Containers: {license.containerNumber}</span>{getStatusBadge(license.status)}</div>}</header>
      <TabButtons />
      <div className="tab-content">{activeTab === "general" && <GeneralTab licenseId={licenseId} license={license} onRefresh={loadLicense} />}{activeTab === "tasks" && <TasksTab licenseId={licenseId} licenseType="virtuozzo" />}</div>
    </div>
  );
}
