// ============================================================
// SQLSERVER TASKS TAB - Composant isolé
// ============================================================
...
/** Historique des tâches d'une licence SQL Server. */
export default function TasksTab({ licenseId }: TasksTabProps) {
  ...
  return (
    <div className="sqlserver-tasks-tab">
      ...
      {tasks.length === 0 ? (
        <div className="sqlserver-tasks-empty">
          <p>{t("tasks.empty")}</p>
        </div>
      ) : (
          ...
          <tbody>
            {tasks.map((task) => (
              <tr key={task.id}>
                ...
                <td>{task.action}</td>
                <td>
                  <span className={`sqlserver-tasks-status-badge ${getStatusBadgeClass(task.status)}`}>
