// ============================================================
// CPANEL TASKS TAB - Composant isolé
// ============================================================
...
export default function TasksTab({ licenseId }: TasksTabProps) {
  ...
  if (error) {
    return (
      <div className="cpanel-tasks-error-state">
        <p>{error}</p>
  ...
  return (
    <div className="cpanel-tasks-tab">
      ...
      {tasks.length === 0 ? (
        <div className="cpanel-tasks-empty">
          <p>{t("empty")}</p>
        </div>
      ) : (
          ...
          <tbody>
            {tasks.map((task) => (
              <tr key={task.id}>
                ...
                <td>{task.action}</td>
                <td>
                  <span className={`cpanel-tasks-status-badge ${getStatusBadgeClass(task.status)}`}>
