...
interface CpanelLicense { id: string; ip: string; version: string; status: string; createdAt: string; }
interface GeneralTabProps { licenseId: string; license: CpanelLicense | null; onRefresh: () => void; }
...
export default function GeneralTab({ licenseId, license, onRefresh }: GeneralTabProps) {
  ...
  return (
    <div className="general-tab">
      <div className="tab-toolbar"><h2>{t("cpanel.general.title")}</h2><button className="btn btn-outline" onClick={onRefresh}>{tCommon("actions.refresh")}</button></div>
