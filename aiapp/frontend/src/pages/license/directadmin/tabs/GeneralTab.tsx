...
interface GeneralTabProps { licenseId: string; license: DirectAdminLicense | null; onRefresh: () => void; }
export default function GeneralTab({ licenseId, license, onRefresh }: GeneralTabProps) {
  ...
  return (
    <div className="general-tab">
      <div className="tab-toolbar"><h2>{t("directadmin.general.title")}</h2><button className="btn btn-outline" onClick={onRefresh}>{tCommon("actions.refresh")}</button></div>
      <div className="license-info-card"><div className="info-grid">
        ...
        <div className="info-item"><span className="info-label">{t("directadmin.general.fields.os")}</span><span className="info-value">{license.os}</span></div>
