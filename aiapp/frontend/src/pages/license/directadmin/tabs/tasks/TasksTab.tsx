// ============================================================
// DIRECTADMIN TASKS TAB - Composant STRICTEMENT isolé
// ============================================================
...
export default function TasksTab({ licenseId }: TasksTabProps) {
  const { t } = useTranslation("license/directadmin/tasks");
  ...
  if (loading) {
    return <div className="directadmin-tasks-loading-state">{tCommon("loading")}</div>;
  }
  ...
  return (
    <div className="directadmin-tasks-tab">
      <div className="directadmin-tasks-toolbar">
        <h2>{t("title")}</h2>
        <button className="directadmin-tasks-btn directadmin-tasks-btn-outline" onClick={loadTasks}>
          {tCommon("actions.refresh")}
        </button>
      </div>

      {tasks.length === 0 ? (
        <div className="directadmin-tasks-empty-state">{t("empty")}</div>
      ) : (
        <div className="directadmin-tasks-table-container">
            ...
            <tbody>
              {tasks.map((task) => (
                <tr key={task.id}>
                  ...
                  <td>{task.action}</td>
                  <td>
                    <span className={`directadmin-tasks-status-badge directadmin-tasks-status-${task.status}`}>
