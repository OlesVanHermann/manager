...
import * as indicesService from "./IndicesTab";
...
export default function IndicesTab({ serviceId }: IndicesTabProps) {
  ...
  return (
    <div className="indices-tab">
      <div className="indices-toolbar"><h2>{t("indices.title")}</h2><button className="btn btn-primary">{t("indices.create")}</button></div>
      {indices.length === 0 ? (
        <div className="indices-empty-state"><h2>{t("indices.empty.title")}</h2><p>{t("indices.empty.description")}</p></div>
      ) : (
        <table className="indices-table">
          <thead><tr><th>{t("indices.columns.name")}</th><th>{t("indices.columns.shards")}</th><th>{t("indices.columns.usage")}</th><th>{t("indices.columns.created")}</th><th>{t("indices.columns.actions")}</th></tr></thead>
          <tbody>
            {indices.map((idx) => {
