...
import * as aliasesService from "./AliasesTab";
...
export default function AliasesTab({ serviceId }: AliasesTabProps) {
  ...
  return (
    <div className="aliases-tab">
      <div className="aliases-toolbar"><h2>{t("aliases.title")}</h2><button className="btn btn-primary">{t("aliases.create")}</button></div>
      {aliases.length === 0 ? (
        <div className="aliases-empty-state"><h2>{t("aliases.empty.title")}</h2><p>{t("aliases.empty.description")}</p></div>
      ) : (
        <table className="aliases-table">
          <thead><tr><th>{t("aliases.columns.name")}</th><th>{t("aliases.columns.description")}</th><th>{t("aliases.columns.indices")}</th><th>{t("aliases.columns.streams")}</th><th>{t("aliases.columns.actions")}</th></tr></thead>
          <tbody>
            {aliases.map((a) => (
              <tr key={a.aliasId}>
