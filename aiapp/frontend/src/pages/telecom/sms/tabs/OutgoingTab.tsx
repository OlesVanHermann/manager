...
import { telecomService, SmsOutgoing } from "../../../../services/telecom.service";
...
export function OutgoingTab({ serviceName }: Props) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const ids = await telecomService.listSmsOutgoing(serviceName);
        const data = await Promise.all(ids.slice(0, 50).map(id => telecomService.getSmsOutgoing(serviceName, id)));
