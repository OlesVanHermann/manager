...
import { telecomService, TelephonyVoicemail } from "../../../../services/telecom.service";

interface Props { billingAccount: string; }

export function VoicemailsTab({ billingAccount }: Props) {
  const { t } = useTranslation("telecom/voip/index");
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const names = await telecomService.listVoicemails(billingAccount);
        const data = await Promise.all(names.map(n => telecomService.getVoicemail(billingAccount, n)));
        setVoicemails(data);
