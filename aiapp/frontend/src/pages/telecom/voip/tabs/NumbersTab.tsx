...
import { telecomService, TelephonyNumber } from "../../../../services/telecom.service";
...
export function NumbersTab({ billingAccount }: Props) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const names = await telecomService.listNumbers(billingAccount);
        const data = await Promise.all(names.map(n => telecomService.getNumber(billingAccount, n)));
        setNumbers(data);
