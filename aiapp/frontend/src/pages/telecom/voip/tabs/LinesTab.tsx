...
import { telecomService, TelephonyLine } from "../../../../services/telecom.service";
...
export function LinesTab({ billingAccount }: Props) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const names = await telecomService.listLines(billingAccount);
        const data = await Promise.all(names.map(n => telecomService.getLine(billingAccount, n)));
        setLines(data);
