...
export function BackupsTab({ serviceName }: Props) {
  ...
  if (loading) return <div className="mwp-backups-loading"><div className="mwp-backups-skeleton" /></div>;
  if (error) return <div className="mwp-backups-error">{error}</div>;

  return (
    <div className="backups-tab">
      <div className="backups-header">
        <div><h3>{t("backups.title")}</h3><p className="backups-description">{t("backups.description")}</p></div>
        <div className="backups-actions"><button className="mwp-backups-btn-primary" onClick={handleCreateBackup} disabled={creating}>{creating ? "Création..." : `+ ${t("backups.create")}`}</button></div>
      </div>
      <div className="backups-info-banner"><span>ℹ</span><p>{t("backups.info")}</p></div>
      {backups.length === 0 ? (
        <div className="backups-empty"><span className="backups-empty-icon">💾</span><h4>{t("backups.empty")}</h4><p>{t("backups.emptyHint")}</p><button className="mwp-backups-btn-primary" onClick={handleCreateBackup} disabled={creating}>{creating ? "..." : `+ ${t("backups.create")}`}</button></div>
      ) : (
          ...
          <tbody>
            {backups.map(b => (
              <tr key={b.id}>
                ...
                <td><span className={`mwp-backups-badge ${b.type === "automatic" ? "info" : "secondary"}`}>{b.type === "automatic" ? "Automatique" : "Manuel"}</span></td>
                <td>{formatSize(b.size)}</td>
                <td><span className={`mwp-backups-badge ${getStatusBadge(b.status).class}`}>{getStatusBadge(b.status).label}</span></td>
