...
export function ThemesPluginsTab({ serviceName }: Props) {
  ...
  const [themes, setThemes] = useState<ManagedWordPressTheme[]>([]);
  const [plugins, setPlugins] = useState<ManagedWordPressPlugin[]>([]);
  ...
  const handleTogglePlugin = async (p: ManagedWordPressPlugin) => { setUpdating(`toggle-${p.name}`); try { p.active ? await themesPluginsService.deactivatePlugin(serviceName, p.name) : await themesPluginsService.activatePlugin(serviceName, p.name); loadData(); } catch (err) { alert(`Erreur: ${err}`); } finally { setUpdating(null); } };
    ...
    <div className="themes-plugins-tab">
          ...
          <div className="themes-plugins-grid">
            {themes.map(theme => (
              <div key={theme.name} className={`themes-plugins-card ${theme.active ? "active" : ""}`}>
                ...
                <div className="themes-plugins-info"><h5>{theme.name}</h5><span className="themes-plugins-version">v{theme.version}</span>{theme.active && <span className="badge success">Actif</span>}</div>
                {theme.updateAvailable && <button className="btn btn-sm btn-warning" onClick={() => handleUpdateTheme(theme.name)} disabled={updating === `theme-${theme.name}`}>{updating === `theme-${theme.name}` ? "..." : "⬆️"}</button>}
              </div>
            ))}
          </div>
        )}
      ...
      <section className="themes-plugins-section">
        <div className="themes-plugins-section-header"><h4>{t("plugins.title")}</h4><span className="themes-plugins-count-badge">{plugins.length}</span>
          {pluginsWithUpdates.length > 0 && <button className="btn btn-sm btn-warning" onClick={handleUpdateAll} disabled={updating === "all"}>{updating === "all" ? "..." : `⬆️ ${t("plugins.updateAll")} (${pluginsWithUpdates.length})`}</button>}
        </div>
        {plugins.length === 0 ? <div className="themes-plugins-empty">{t("plugins.empty")}</div> : (
          <table className="themes-plugins-table">
            ...
            <tbody>
              {plugins.map(plugin => (
                <tr key={plugin.name}>
                  <td><span className="themes-plugins-name">{plugin.name}{plugin.updateAvailable && <span className="badge warning" title="Mise à jour disponible">⬆️</span>}</span></td>
                  <td><code>{plugin.version}</code></td>
                  <td><span className={`badge ${plugin.active ? "success" : "inactive"}`}>{plugin.active ? t("plugins.active") : t("plugins.inactive")}</span></td>
