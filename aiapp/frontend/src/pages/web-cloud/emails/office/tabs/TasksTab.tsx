// ============================================================
// OFFICE TAB: TASKS (style Hosting)
// ============================================================
...
export function TasksTab({ tenantId }: Props) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const ids = await officeService.listTasks(tenantId);
        const data = await Promise.all(ids.slice(0, 50).map(id => officeService.getTask(tenantId, id)));
