...
export const modemOvhService = {
  ...
  /** Modifie la configuration générale du modem (MTU, bridge, firewall, etc.). */
  async updateModemConfig(connectionId: string, config: {
    mtuSize?: number;
    isBridged?: boolean;
    sipAlg?: boolean;
    dmzIP?: string | null;
    ...
    const accessNames = await ovhApi.get<string[]>(`/pack/xdsl/${connectionId}/xdslAccess/services`);
    return ovhApi.put<Task>(`/xdsl/${accessNames[0]}/modem`, config);
  },

  /** Active/désactive la gestion à distance par OVH. */
  async setManagedByOvh(connectionId: string, managed: boolean): Promise<Task> {
    return this.updateModemConfig(connectionId, { managedByOvh: managed });
  },

  /** Configure le mode bridge. */
  async setBridgeMode(connectionId: string, enabled: boolean): Promise<Task> {
    return this.updateModemConfig(connectionId, { isBridged: enabled });
  },

  /** Configure le MTU. */
  async setMtu(connectionId: string, mtuSize: number): Promise<Task> {
    return this.updateModemConfig(connectionId, { mtuSize });
  },

  /** Configure le niveau du firewall. */
  async setFirewallLevel(connectionId: string, level: 'disabled' | 'normal' | 'high'): Promise<Task> {
    return this.updateModemConfig(connectionId, { easyFirewallLevel: level });
  },

  /** Configure la DMZ. */
  async setDmz(connectionId: string, dmzIP: string | null): Promise<Task> {
    return this.updateModemConfig(connectionId, { dmzIP });
  },
};
