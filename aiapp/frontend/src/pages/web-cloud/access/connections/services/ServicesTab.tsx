  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        setError(null);
        const data = await connectionsService.getServices(connectionId);
        setServices(data);
      } catch (err) {
        setError(String(err));
      } finally {
  ...
  return (
    <div className="services-tab">
      <div className="services-header">
        ...
        <span className="services-count">{services.length} {t("services.total")}</span>
      </div>
      ...
      {services.length === 0 ? (
        <div className="services-empty">
          ...
          <button className="btn-primary">{t("services.discover")}</button>
        </div>
      ) : (
                ...
                {items.map((service) => (
                  <div key={service.id} className="service-item">
                    ...
                    <div className="service-actions">
                      <button className="btn-link">{t("services.manage")}</button>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <div className="services-footer">
        <p>{t("services.needMore")}</p>
        <button className="btn-secondary">{t("services.addService")}</button>
      </div>
    </div>
  );
}
