...
import { ModemOvhTab } from "./modem-ovh/ModemOvhTab";
import { ModemPersoTab } from "./modem-perso/ModemPersoTab";
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        setError(null);
        const data = await connectionsService.getConnection(connectionId);
        setConnection(data);
      } catch (err) {
        setError(String(err));
      } finally {
