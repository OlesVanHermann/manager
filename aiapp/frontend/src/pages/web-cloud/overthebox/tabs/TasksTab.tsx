...
interface Task { id: string; name: string; status: string; todoDate: string; doneDate?: string; }
...
export default function TasksTab({ serviceId }: TasksTabProps) {
  ...
  const loadTasks = async () => {
    try { setLoading(true); setError(null); const data = await otbService.getTasks(serviceId); setTasks(data); }
    ...
    <div className="tasks-tab">
      ...
      {tasks.length === 0 ? <div className="empty-state"><h2>{t("tasks.empty.title")}</h2></div> : (
        <table className="data-table">
          <thead><tr><th>{t("tasks.columns.name")}</th><th>{t("tasks.columns.status")}</th><th>{t("tasks.columns.scheduled")}</th><th>{t("tasks.columns.completed")}</th></tr></thead>
          <tbody>
            {tasks.map((task) => (<tr key={task.id}><td>{task.name}</td><td>{getStatusBadge(task.status)}</td><td>{new Date(task.todoDate).toLocaleString("fr-FR")}</td><td>{task.doneDate ? new Date(task.doneDate).toLocaleString("fr-FR") : "-"}</td></tr>))}
          </tbody>
        </table>
      )}
    </div>
