...
import { domainsService, Redirection, RedirectionCreate } from "../../../../services/web-cloud.domains";
...
export function RedirectionTab({ domain }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (modalMode === 'create') {
        ...
        await domainsService.createRedirection(domain, payload);
      } else if (formData.id) {
        await domainsService.updateRedirection(domain, formData.id, {
          target: formData.target,
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setDeleting(true);
      await domainsService.deleteRedirection(domain, deleteConfirm.id);
      await domainsService.refreshZone(domain);
