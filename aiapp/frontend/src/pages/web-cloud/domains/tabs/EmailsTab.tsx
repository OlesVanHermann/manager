...
export function EmailsTab({ domain }: Props) {
  ...
  const loadData = useCallback(async () => {
    try { setLoading(true); const [a, m, r] = await Promise.all([domainsService.listEmailAccounts(domain), domainsService.listMailingLists(domain), domainsService.listEmailRedirections(domain)]); setAccounts(a); setMailingLists(m); setRedirections(r); setHasEmailService(a.length > 0 || m.length > 0 || r.length > 0); }
