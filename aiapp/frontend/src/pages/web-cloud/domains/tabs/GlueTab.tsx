...
export function GlueTab({ domain }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setFormError(null);
      if (formData.isEdit) {
        await domainsService.updateGlueRecord(domain, formData.host, ips);
      } else {
        const fullHost = formData.host.includes('.') ? formData.host : `${formData.host}.${domain}`;
        await domainsService.createGlueRecord(domain, { host: fullHost, ips });
      }
      closeModal();
      await loadRecords();
    } catch (err) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setDeleting(true);
      await domainsService.deleteGlueRecord(domain, deleteConfirm.host);
