...
import { domainsService, DynHostRecord, DynHostLogin } from "../../../../services/web-cloud.domains";
...
export function DynHostTab({ zoneName }: Props) {
  ...
  const loadLogins = useCallback(async () => {
    try {
      ...
      const loginNames = await domainsService.listDynHostLogins(zoneName);
      if (loginNames.length === 0) {
        setLogins([]);
        return;
      }
      const details = await Promise.all(loginNames.map((login) => domainsService.getDynHostLogin(zoneName, login)));
      setLogins(details);
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setFormError(null);
      if (modalMode === "create") {
        await domainsService.createDynHostRecord(zoneName, { subDomain: formData.subDomain, ip: formData.ip.trim() });
      } else if (formData.id) {
        await domainsService.updateDynHostRecord(zoneName, formData.id, { ip: formData.ip.trim() });
      }
      closeModal();
      await loadRecords();
    } catch (err) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setDeleting(true);
      await domainsService.deleteDynHostRecord(zoneName, deleteConfirm.id);
  ...
  const handleSaveLogin = async () => {
    ...
    try {
      setSavingLogin(true);
      setLoginFormError(null);
      await domainsService.createDynHostLogin(zoneName, {
  ...
  const handleDeleteLoginConfirm = async () => {
    ...
    try {
      setDeleting(true);
      await domainsService.deleteDynHostLogin(zoneName, deleteLoginConfirm.login);
