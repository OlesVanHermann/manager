...
import type { WordPressTheme, WordPressPlugin, WordPressTask } from "../../wordpress.types";
...
export const themesPluginsService = {
  ...
  async updateTheme(serviceName: string, themeName: string): Promise<WordPressTask> {
    return apiClient.post(`${BASE_PATH}/${serviceName}/theme/${themeName}/update`, {}, API_OPTIONS);
  },

  async updatePlugin(serviceName: string, pluginName: string): Promise<WordPressTask> {
  ...
  async updateAllPlugins(serviceName: string): Promise<WordPressTask> {
