...
export function OrderSslModal({ serviceName, isOpen, onClose, onSuccess }: Props) {
  ...
  const handleOrder = async () => { console.log('[Modal:OrderSsl-sites] Action: commander', { serviceName }); setLoading(true); try { await new Promise(r => setTimeout(r, 500)); onSuccess(); } catch {} finally { setLoading(false); } };
