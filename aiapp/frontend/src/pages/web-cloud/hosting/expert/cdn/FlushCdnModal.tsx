...
export function FlushCdnModal({ serviceName, isOpen, onClose, onSuccess }: Props) {
  ...
  const handleFlush = async () => { console.log('[Modal:FlushCdn] Action: vider cache', { serviceName }); setLoading(true); try { await new Promise(r => setTimeout(r, 500)); onSuccess(); } catch {} finally { setLoading(false); } };
