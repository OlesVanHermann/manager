...
export function CopyDatabaseModal({ serviceName, databaseName, isOpen, onClose, onSuccess }: Props) {
  ...
  const handleCopy = async () => { console.log('[Modal:CopyDatabase] Action: copier', { serviceName, databaseName }); setLoading(true); try { await new Promise(r => setTimeout(r, 500)); onSuccess(); } catch {} finally { setLoading(false); } };
  return (
