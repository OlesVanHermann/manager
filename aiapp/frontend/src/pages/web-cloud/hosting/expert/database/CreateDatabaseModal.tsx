...
export function CreateDatabaseModal({ serviceName, isOpen, onClose, onSuccess }: Props) {
  ...
  const handleCreate = async () => { console.log('[Modal:CreateDatabase] Action: créer', { serviceName }); setLoading(true); try { await new Promise(r => setTimeout(r, 500)); onSuccess(); } catch {} finally { setLoading(false); } };
  return (
