...
export function ImportSqlModal({ serviceName, databaseName, isOpen, onClose, onSuccess }: Props) {
  ...
  const handleImport = async () => { console.log('[Modal:ImportSql] Action: importer', { serviceName, databaseName }); setLoading(true); try { await new Promise(r => setTimeout(r, 500)); onSuccess(); } catch {} finally { setLoading(false); } };
  return (
