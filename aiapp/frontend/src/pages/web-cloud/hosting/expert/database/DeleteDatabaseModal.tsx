...
export function DeleteDatabaseModal({ serviceName, databaseName, isOpen, onClose, onSuccess }: Props) {
  ...
  const handleDelete = async () => { console.log('[Modal:DeleteDatabase] Action: supprimer', { serviceName, databaseName }); setLoading(true); try { await new Promise(r => setTimeout(r, 500)); onSuccess(); } catch {} finally { setLoading(false); } };
