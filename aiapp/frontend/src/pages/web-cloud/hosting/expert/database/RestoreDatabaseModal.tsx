...
export function RestoreDatabaseModal({ serviceName, databaseName, isOpen, onClose, onSuccess }: Props) {
  ...
  const handleRestore = async () => { console.log('[Modal:RestoreDatabase] Action: restaurer', { serviceName, databaseName }); setLoading(true); try { await new Promise(r => setTimeout(r, 500)); onSuccess(); } catch {} finally { setLoading(false); } };
  return (
