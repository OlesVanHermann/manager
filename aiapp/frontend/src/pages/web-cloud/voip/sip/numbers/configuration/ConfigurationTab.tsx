...
import { configurationTabService, type NumberConference } from './ConfigurationTab.service';
...
export function ConfigurationTab({ billingAccount, serviceName, number }: ConfigurationTabProps) {
  const { t } = useTranslation('web-cloud/voip/numbers/configuration');
  ...
  const loadConfiguration = async () => {
    try {
      ...
      if (number.featureType === 'conference') {
        const conf = await configurationTabService.getConference(billingAccount, serviceName);
        setConference(conf);
      }
    } catch {
      // Erreur silencieuse
      ...
      <Tile title={t('current.title')}>
        <div className="current-config">
          <div className="current-icon">
            ...
            {!['redirect', 'conference', 'svi', 'voicemail', 'ddi'].includes(number.featureType) && '⚙️'}
          </div>
