...
import { dashboardTabService, type NumberCall } from './DashboardTab.service';
...
export function DashboardTab({ billingAccount, serviceName, number }: DashboardTabProps) {
  const { t } = useTranslation('web-cloud/voip/numbers/dashboard');
  ...
  useEffect(() => {
    const loadDashboard = async () => {
      try {
        setLoading(true);
        const calls = await dashboardTabService.getCalls(billingAccount, serviceName);
  ...
  const quickActions = [
    {
      ...
      onClick: () => navigate(`/web-cloud/voip/${billingAccount}/number/${serviceName}/configuration`),
    },
    {
      label: t('quickActions.scheduler'),
      onClick: () => navigate(`/web-cloud/voip/${billingAccount}/number/${serviceName}/scheduler`),
    },
    {
      label: t('quickActions.records'),
      onClick: () => navigate(`/web-cloud/voip/${billingAccount}/number/${serviceName}/records`),
    },
    {
      label: t('quickActions.consumption'),
      onClick: () => navigate(`/web-cloud/voip/${billingAccount}/number/${serviceName}/consumption`),
    },
  ];
    ...
    <div className="number-dashboard-tab">
      ...
      <div className="voip-tiles-row">
        ...
        <Tile title={t('info.title')}>
          ...
          <InfoRow
            label={t('info.pool')}
            value={number.partOfPool ? `#${number.partOfPool}` : '-'}
          />
        </Tile>
        ...
        <Tile title={t('config.title')}>
          <div className="config-visual">
            <div className="config-icon">
              ...
              {number.featureType === 'svi' && '🔢'}
              {number.featureType === 'voicemail' && '📧'}
              {!['redirect', 'conference', 'svi', 'voicemail'].includes(number.featureType) && '📞'}
            </div>
