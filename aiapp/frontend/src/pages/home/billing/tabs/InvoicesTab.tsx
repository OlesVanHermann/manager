...
/** Affiche l'historique des factures avec filtrage par période, chargement streaming, et export CSV/PDF. */
export function InvoicesTab({ credentials }: TabProps) {
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `OVHcloud-${t('invoices.exportFilename')}_${nav.year}-${String(nav.startMonth + 1).padStart(2, "0")}_${nav.year}-${String(nav.endMonth + 1).padStart(2, "0")}.csv`;
    a.click();
  ...
  return (
    <div className="tab-panel">
      <PeriodToolbar
        ...
        countLabelKey="invoices.countProgress"
      />
