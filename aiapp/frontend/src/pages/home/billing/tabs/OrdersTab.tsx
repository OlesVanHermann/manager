// ============================================================
// ORDERS TAB - Commandes avec chargement streaming + fallback
// ============================================================
...
interface OrderRow {
  orderId: number;
  loaded: boolean;
  details?: ordersService.OrderWithStatus;
}

// ============ COMPOSANT ============

/** Affiche l'historique des commandes avec filtrage par période, chargement streaming, et export CSV/PDF. */
export function OrdersTab({ credentials }: TabProps) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status?.toLowerCase()) {
      ...
      case "notpaid": return <span className="status-badge badge-warning">{t('orders.status.notPaid')}</span>;
    ...
    setOrders((prev) => {
      ...
      for (const r of results) {
        if (r.success && r.details) {
          next.set(r.id, { orderId: r.id, loaded: true, details: r.details });
        } else {
          next.set(r.id, { orderId: r.id, loaded: true });
        }
      }
      return next;
    });
  ...
  const loadOrders = useCallback(async () => {
    ...
    try {
      ...
      const initial = new Map<number, OrderRow>();
      for (const id of sortedIds) {
        initial.set(id, { orderId: id, loaded: false });
      }
      setOrders(initial);
      setLoadingIds(false);
  ...
  const exportCSV = () => {
    ...
    const rows = data.map(o => [
      o.orderId.toString(),
      formatDateISO(o.date),
      (o.priceWithoutTax?.value || 0).toFixed(2),
      (o.priceWithTax?.value || 0).toFixed(2),
      o.priceWithTax?.currencyCode || "EUR",
      o.status
    ]);
    ...
    a.href = url;
    a.download = `OVHcloud-${t('orders.exportFilename')}_${nav.year}-${String(nav.startMonth + 1).padStart(2, "0")}_${nav.year}-${String(nav.endMonth + 1).padStart(2, "0")}.csv`;
    a.click();
  ...
  return (
    <div className="tab-panel">
      <PeriodToolbar
        ...
        countLabelKey="orders.countProgress"
      />

      {allLoaded && orderIds.length > 0 && (
      ...
      {loadingIds ? (
        ...
        <div className="table-container">
          <table className="data-table">
            ...
            <tbody>
              {finalRows.map((row) => (
                <tr key={row.orderId} className={row.loaded ? "" : "loading-row"}>
                  <td className="ref-cell"><span className="ref-badge">{row.orderId}</span></td>
                  <td>{row.loaded && row.details ? formatDate(row.details.date) : <Skeleton />}</td>
                  <td className="amount-cell">{row.loaded && row.details ? formatAmount(row.details.priceWithoutTax?.value || 0, row.details.priceWithoutTax?.currencyCode || "EUR") : <Skeleton />}</td>
                  <td className="amount-cell">{row.loaded && row.details ? formatAmount(row.details.priceWithTax?.value || 0, row.details.priceWithTax?.currencyCode || "EUR") : <Skeleton />}</td>
                  <td>{row.loaded && row.details ? getStatusBadge(row.details.status) : <Skeleton />}</td>
