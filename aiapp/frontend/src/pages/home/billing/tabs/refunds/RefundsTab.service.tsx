// ============================================================
// REFUNDS TAB SERVICE - Service ISOLÉ
// ============================================================
  ...
  return (
    <div className="refunds-toolbar">
      <div className="refunds-toolbar-left">
        <span className="refunds-year-label">{year}</span>
        <button className="btn refunds-year-nav-btn" onClick={goToPrevious} disabled={!canGoPrevious} title={t('invoices.nav.previous')}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#333" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M15 18l-6-6 6-6"/></svg></button>
        <select className="refunds-period-select refunds-month-select" value={startMonth} onChange={(e) => handleStartMonthChange(Number(e.target.value))}>{MONTHS_SHORT.map((m, i) => <option key={`start-${i}`} value={i}>{m}</option>)}</select>
        <span className="refunds-date-separator">→</span>
        <select className="refunds-period-select refunds-month-select" value={endMonth} onChange={(e) => handleEndMonthChange(Number(e.target.value))}>{MONTHS_SHORT.map((m, i) => <option key={`end-${i}`} value={i}>{m}</option>)}</select>
        <button className="btn refunds-year-nav-btn" onClick={goToNext} disabled={!canGoNext} title={t('invoices.nav.next')}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#333" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M9 18l6-6-6-6"/></svg></button>
        {showReset && <button className="btn refunds-reset-btn" onClick={resetToAnchor} title={t('invoices.nav.reset')}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#333" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/></svg></button>}
      </div>
      <div className="refunds-toolbar-right">
        {allLoaded && totalCount > 0 && (<><button className="btn btn-sm btn-secondary refunds-export-btn" onClick={onExportCSV} title={t('invoices.export.csv')}><FileTextIcon /> CSV</button><button className="btn btn-sm btn-secondary refunds-export-btn" onClick={onExportPDF} title={t('invoices.export.pdf')}><FileIcon /> PDF</button></>)}
        <span className="refunds-result-count">{loadingIds ? tCommon('loading') : t(countLabelKey, { loaded: loadedCount, total: totalCount })}</span>
      </div>
    </div>
  );
}
