...
/** Affiche l'historique des paiements avec filtrage par période, chargement streaming, et export CSV/PDF. */
export function PaymentsTab({ credentials }: TabProps) {
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `OVHcloud-${t('payments.exportFilename')}_${nav.year}-${String(nav.startMonth + 1).padStart(2, "0")}_${nav.year}-${String(nav.endMonth + 1).padStart(2, "0")}.csv`;
    a.click();
  ...
  return (
    <div className="tab-panel">
      <PeriodToolbar
        ...
        totalCount={depositIds.length}
        allLoaded={allLoaded}
        onExportCSV={exportCSV}
        onExportPDF={exportPDF}
        countLabelKey="payments.countProgress"
      />
