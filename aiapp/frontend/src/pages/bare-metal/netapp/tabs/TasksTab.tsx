...
interface Task { id: string; type: string; status: string; progress: number; createdAt: string; finishedAt?: string; }
...
export default function TasksTab({ serviceId }: TasksTabProps) {
  ...
  const loadTasks = async () => {
    try { setLoading(true); setError(null); const data = await netappService.getTasks(serviceId); setTasks(data); }
    catch (err) { setError(err instanceof Error ? err.message : "Erreur"); }
    finally { setLoading(false); }
  };

  const getStatusBadge = (status: string) => {
    const classes: Record<string, string> = { done: "badge-success", running: "badge-info", pending: "badge-warning", error: "badge-error" };
    ...
    <div className="tasks-tab">
      ...
      {tasks.length === 0 ? <div className="empty-state"><h2>{t("tasks.empty.title")}</h2></div> : (
        <table className="data-table">
          ...
          <tbody>
            {tasks.map((task) => (<tr key={task.id}><td>{task.type}</td><td>{getStatusBadge(task.status)}</td><td>{task.progress}%</td><td>{new Date(task.createdAt).toLocaleString("fr-FR")}</td><td>{task.finishedAt ? new Date(task.finishedAt).toLocaleString("fr-FR") : "-"}</td></tr>))}
          </tbody>
        </table>
      )}
    </div>
