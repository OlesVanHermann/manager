...
interface Volume { id: string; name: string; protocol: string; size: number; usedSize: number; status: string; mountPath: string; }
...
export default function VolumesTab({ serviceId }: VolumesTabProps) {
  ...
  const loadVolumes = async () => {
    try { setLoading(true); setError(null); const data = await netappService.getVolumes(serviceId); setVolumes(data); }
  ...
  return (
    <div className="volumes-tab">
      ...
      {volumes.length === 0 ? (
        <div className="empty-state"><h2>{t("volumes.empty.title")}</h2><p>{t("volumes.empty.description")}</p></div>
      ) : (
            ...
            return (
              <div key={volume.id} className="volume-card">
                <div className="volume-header">
                  <div><div className="volume-name">{volume.name}</div><div style={{ fontSize: "var(--font-size-xs)", color: "var(--color-text-secondary)", fontFamily: "var(--font-mono)" }}>{volume.id}</div></div>
                  <span className="volume-protocol">{volume.protocol}</span>
                </div>
                <div className="info-grid" style={{ marginBottom: "var(--space-3)" }}>
                  <div className="info-card"><div className="card-title">{t("volumes.fields.mountPath")}</div><div className="card-value mono" style={{ fontSize: "var(--font-size-sm)" }}>{volume.mountPath}</div></div>
                </div>
