...
export default function SnapshotsTab({ serviceId }: SnapshotsTabProps) {
  ...
  const loadSnapshots = async () => {
    try { setLoading(true); setError(null); setSnapshots(await snapshotsService.getSnapshots(serviceId)); }
    catch (err) { setError(err instanceof Error ? err.message : "Erreur"); }
    finally { setLoading(false); }
  };

  if (loading) return <div className="netapp-snapshots-loading">{tCommon("loading")}</div>;
  if (error) return <div className="netapp-snapshots-error"><p>{error}</p><button className="btn btn-primary" onClick={loadSnapshots}>{tCommon("actions.retry")}</button></div>;

  return (
    <div className="netapp-snapshots-tab">
      <div className="netapp-snapshots-toolbar"><h2>{t("snapshots.title")}</h2><button className="btn btn-primary">{t("snapshots.create")}</button></div>
      {snapshots.length === 0 ? <div className="netapp-snapshots-empty"><h2>{t("snapshots.empty.title")}</h2></div> : (
        <table className="netapp-snapshots-table">
          ...
          <tbody>{snapshots.map((s) => (<tr key={s.id}><td className="mono">{s.name}</td><td>{s.volumeName}</td><td>{formatDate(s.createdAt)}</td><td className="netapp-snapshots-actions"><button className="btn btn-sm btn-outline">{t("snapshots.restore")}</button><button className="btn btn-sm btn-outline btn-danger">{tCommon("actions.delete")}</button></td></tr>))}</tbody>
        </table>
      )}
    </div>
  );
