...
import { vpsService, VpsTask } from "../../../../services/bare-metal.vps";
...
export function TasksTab({ serviceName }: Props) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const ids = await vpsService.listTasks(serviceName);
        const data = await Promise.all(ids.slice(0, 50).map(id => vpsService.getTask(serviceName, id)));
  ...
  const getStatusBadge = (state: string) => {
    const map: Record<string, { class: string; icon: string }> = { todo: { class: 'warning', icon: '⏳' }, doing: { class: 'info', icon: '🔄' }, done: { class: 'success', icon: '✓' }, error: { class: 'error', icon: '✗' }, cancelled: { class: 'inactive', icon: '⊘' }, paused: { class: 'warning', icon: '⏸' } };
    ...
    <div className="tasks-tab">
      ...
      {tasks.length === 0 ? (<div className="empty-state"><p>{t("tasks.empty")}</p></div>) : (
        <table className="data-table">
          <thead><tr><th>{t("tasks.type")}</th><th>{t("tasks.status")}</th><th>{t("tasks.progress")}</th><th>{t("tasks.started")}</th><th>{t("tasks.done")}</th></tr></thead>
          <tbody>
            {tasks.map(task => (
              <tr key={task.id}>
