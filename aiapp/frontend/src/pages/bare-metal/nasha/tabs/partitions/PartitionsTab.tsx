...
export default function PartitionsTab({ serviceId }: PartitionsTabProps) {
  ...
  const loadPartitions = async () => {
    try { setLoading(true); setError(null); const data = await partitionsService.getPartitions(serviceId); setPartitions(data); }
  ...
  return (
    <div className="partitions-tab">
      <div className="tab-toolbar"><h2>{t("partitions.title")}</h2><button className="btn btn-primary">{t("partitions.create")}</button></div>
      {partitions.length === 0 ? (
        <div className="empty-state"><h2>{t("partitions.empty.title")}</h2><p>{t("partitions.empty.description")}</p></div>
      ) : (
