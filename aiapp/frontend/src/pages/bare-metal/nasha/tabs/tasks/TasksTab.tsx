    ...
    <div className="tasks-tab">
      ...
      {tasks.length === 0 ? (
        ...
        <table className="tasks-data-table">
          ...
          <tbody>
            {tasks.map((task) => (
              <tr key={task.taskId}><td>{task.operation}</td><td>{task.partitionName || "-"}</td><td>{getStatusBadge(task.status)}</td><td>{new Date(task.startDate).toLocaleString("fr-FR")}</td><td>{task.doneDate ? new Date(task.doneDate).toLocaleString("fr-FR") : "-"}</td></tr>
            ))}
          </tbody>
        </table>
      )}
