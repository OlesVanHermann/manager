...
export default function SnapshotsTab({ serviceId }: SnapshotsTabProps) {
  ...
  const loadSnapshots = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await nashaService.getSnapshots(serviceId);
      setSnapshots(data);
    } catch (err) {
    ...
    <div className="snapshots-tab">
      ...
      {snapshots.length === 0 ? (
        <div className="empty-state">
          <h2>{t("snapshots.empty.title")}</h2>
          <p>{t("snapshots.empty.description")}</p>
        </div>
      ) : (
        <table className="data-table">
          <thead>
            <tr>
              ...
              <th>{t("snapshots.columns.partition")}</th>
              <th>{t("snapshots.columns.type")}</th>
              <th>{t("snapshots.columns.created")}</th>
              <th>{t("snapshots.columns.actions")}</th>
            </tr>
          </thead>
          <tbody>
            {snapshots.map((snapshot, idx) => (
              <tr key={idx}>
                ...
                <td>{getTypeBadge(snapshot.type)}</td>
                <td>{new Date(snapshot.createdAt).toLocaleString("fr-FR")}</td>
