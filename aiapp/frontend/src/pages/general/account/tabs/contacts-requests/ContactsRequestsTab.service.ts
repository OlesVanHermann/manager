...
export async function getContactChanges(): Promise<ContactChange[]> {
  const ids = await getContactChangeRequestIds();
  const requests = await Promise.all(ids.map((id) => ovhGet<ContactChange>(`/me/task/contactChange/${id}`).catch(() => null)));
  return requests
    .filter((r): r is ContactChange => r !== null)
