...
export async function changePassword(credentials: OvhCredentials): Promise<void> {
  try {
    await ovhRequest<void>(credentials, "POST", "/me/changePassword");
  } catch {
    window.location.href = "https://www.ovh.com/auth/?action=forgottenPassword";
    throw new Error("Redirection vers le portail OVH...");
  }
}
...
export async function sendSmsCode(credentials: OvhCredentials, id: number): Promise<void> {
  try {
    ...
    console.warn("sendSmsCode endpoint not available, code was sent at creation");
  }
}
